...
(function initExcursionTopicsModal() {
  function setup() {
    ...
    function showTopicsModal(event) {
      ...
      if (legacyMenu) {
        legacyMenu.classList.add('modal-active');
      }
    }

    function closeTopicsModal() {
      if (legacyMenu) {
        legacyMenu.classList.remove('modal-active');
      }
    }

    const topicButtons = Array.from(document.querySelectorAll('.still__exc-wrap .excursion__link-more'));
    topicButtons.forEach((button) => {
      const cleanButton = button.cloneNode(true);
      button.replaceWith(cleanButton);
      cleanButton.addEventListener('click', showTopicsModal);
    });

    if (closeTopics) {
      closeTopics.addEventListener('click', closeTopicsModal);
    }

    topicsModal.addEventListener('click', (event) => {
      if (event.target === topicsModal) closeTopicsModal();
    });

    document.addEventListener('keydown', (event) => {
      if (event.key === 'Escape' && topicsModal.style.display === 'flex') {
        closeTopicsModal();
      }
    });
  }
...
if(document.getElementById('bookingTime')){
   BOOKING_TIMES = JSON.parse(document.getElementById('bookingTime').dataset.json);
}
...
function resetBookingForm() {
  ...
  if (timeInput) setBookingTime(timeInput.value);
...
function initBookingModal() {
  const bookBtns = document.querySelectorAll('.book-btn_new');
  const bookBtns2 = document.querySelectorAll('.book-btn');
  const overlay = document.getElementById('overlay');
...
(function initLegacyCallModal() {
  const modal = document.getElementById('modal');
  const closeBtn = document.getElementById('closeModal');
  const triggers = document.querySelectorAll('#callBtn, #footerCallBtn');
...
// Инициализация тегов при загрузке DOM
(function() {
  if (document.readyState === 'loading') {
  ...
  function initializeTags() {
    ...
    if (!wrapper) {
      console.error('tagsWrapper не найден');
      return;
    }
  ...
  } // Конец initializeTags
})(); // Конец IIFE

// Остальной код ниже не является частью инициализации тегов

(function initExcursionGallery() {
const wrapper = document.getElementById('tagsWrapper');
...
if (originalMore) {
  ...
  if (!mainImg || !thumbsRow) return;

  const PHOTOS = JSON.parse(thumbsRow.dataset.photos || '[]');
...
(function() {
  ...
  console.log(agreeInput,'agreeInput');
  const BASE_REVIEWS = [
